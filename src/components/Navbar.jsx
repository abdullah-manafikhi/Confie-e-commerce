  ...
  return (
    <nav className='flex flex-auto justify-between px-16 md:px-48 pt-2 text-green text-xl font-bold h-fit'>
      <label className="btn btn-circle swap swap-rotate absolute z-50 left-6 bg-green sm:hidden">
        ...
        <div className={`${navToggle} grid grid-cols-1 mt-16 pl-24 sm:flex sm:justify-between sm:mt-0 sm:pl-8 w-80`}>
            ...
            {displayAbout ? <Link to='/' ref={reference} onClick={() => click()} className='hover:text-emerald-500'>About</Link > : ''}
        </div>
        { 
        //choosing wether to display the login and sign up buttons or the sign up buttons
          ...
          (
            <div className="absolute right-6">
              <span className="font-normal relative -right-2">Hello {userName} !</span>
